...
| AC-33a | Previous discussion list — items render | [§3.3](ACCEPTANCE_CRITERIA_BODIES.md#33-expanded-chat-mode) | active | @evolving | Manual: dev harness with seeded multi-conversation store — each row carries a label derived from its first user question; idle (inactive) uses the Default surface and the active row uses the Hover surface per §2.5 row AC-33a, matching `site:434:2424` (`property1="Hover"` for the active conversation). Row label typography stays Regular on every variant. |
...
| AC-21   | `ds:152:97`, `ds:152:98`, `site:435:2904`     | Expanded header ("Siili AI-avustaja"). Desktop: left-aligned with the sidebar column per `ds:152:97`, font-size 24 px Everett Bold per `ds:152:98`. Mobile (<640 px PD-05): inside the mobile top-bar row co-anchored with the AC-33d hamburger (left) and the AC-20d close button (right) per `site:435:2904`. | 2026-05-13   | Lane P — Investor agent re-sync (title bumped 20 px → 24 px to match `ds:152:98`) |
...
| AC-33a  | `ds:230:452` (parent set: Default `ds:191:268` / Hover `ds:230:453` / Pressed `ds:230:459`), `site:434:2424` | Previous discussion item — row content & visual states. Per-row surface treatment lifted out of the sidebar shell (Lane J). Idle (inactive) rows carry the Default variant's neutral surface; hovered inactive rows step up to the Hover variant's surface. The active conversation row carries the **Hover** variant's surface as its idle treatment per `site:434:2424` (`property1="Hover"` for the active row), one step darker than an inactive Default row. Row label typography stays Everett Regular on every variant — no bold-label cue is rendered; the surface step is the cue. The Pressed variant remains a documented part of the parent set but is not rendered as a static row state by the widget (reserved for a future transient "while pressing" surface if one is ever needed). The trailing × delete button carries Default / Hover / Pressed surface treatments on its 24 × 24 hit target (mirrors `CloseButton`). Row geometry: `px-[16px] py-[4px]` padding, `gap-[16px]` between the activate button and the trailing 24 × 24 reset icon, `rounded-[20px]` (`--radius`). | 2026-05-26   | Designer QA follow-up — × hover states + host focus ring; active row stays Hover per Figma (`site:434:2424`) |
