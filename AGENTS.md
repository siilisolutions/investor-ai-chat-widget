...
| `591:3203` | Etusivu - Mobile — 390×924 mobile compact-hero composition (host's `Header (With menu)` chrome above the existing `ds:152:75` `Investor hero` scaled to 390 wide). Candidate anchor for `AC-92` / `AC-92b`; promotion deferred per `.cursor/rules/ac-amending.mdc` (Lane L surfaced, designer to confirm canonical iteration). |
| `608:1855` | Etusivu - Mobile - v2 — second iteration of the mobile compact-hero composition. Same overall structure as `591:3203`; visually indistinguishable in `get_design_context` renders. Candidate anchor for `AC-92` / `AC-92b`; promotion deferred (Lane L). |
