...
| `237:398` | Start-new-conversation Button — primary CTA "Luo uusi keskustelu" at the top of the sidebar (component set with `Property 1` variants `Default` `237:323` / `Hover` `237:399` / `Pressed` `237:411`); 184×40 violet→blue gradient pill with a leading 11×11 plus icon. Anchors AC-35. Renders today as the in-sidebar button inside `src/components/PreviousDiscussionList.tsx` (gradient + label + Hover / Pressed states do not yet match — see § Known Gaps / TODOs). | part of `src/components/PreviousDiscussionList.tsx` |
