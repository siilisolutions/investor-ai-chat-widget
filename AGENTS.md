...
| `230:656` | Menu button (hamburger — 24×24 with three 16×2.4 px stripes, component set with `Default` / `Hover` / `Pressed` variants). Anchors AC-33d as the affordance that opens the mobile drawer. Rendered only below the §12.1 PD-05 desktop threshold (lg / 1024 px; `ds:152:97` Code Connect). | `src/components/MenuButton.tsx` |
...
| `--gray-600` | `#c0c0c0` | ContinuePill 1 px divider (Figma `site:395:5439`). Also the Figma slot for the Pressed variant of `ds:230:452` (`ds:230:459`); the widget does not render that as a static row state (AC-33a active row uses the Hover surface per `site:434:2424` — see AC-33a amended 2026-05). |
...
| `src/components/ExpandedView.tsx` | Chat mode: messages list + input + close button + sidebar (2-column layout, sidebar always rendered per AC-33 amended 2026-05). Below the §12.1 PD-05 desktop threshold (lg / 1024 px; Figma `ds:152:97` Code Connect) the sidebar collapses behind a hamburger toggle (AC-33d, Figma `ds:230:656`) that opens the `MobileMenu` drawer (`ds:214:1214`); the close button moves into the mobile top-bar row alongside the hamburger and title per `site:435:2904`. |
...
| `src/components/CloseButton.tsx` | × button rendered top-right of expanded view on desktop ≥lg (AC-20d, Figma `ds:196:853`); moves into the mobile top-bar row below lg (1024 px) per `site:435:2904` / `ds:152:97` Code Connect. The same component also serves as the AC-33d drawer's internal × inside `MobileMenu`. |
| `src/components/MenuButton.tsx` | Hamburger toggle (24×24 with three 16×2.4 px stripes) that opens the AC-33d mobile drawer. Maps to Figma `ds:230:656`; rendered only below the §12.1 PD-05 desktop threshold (lg) via CSS. |
...
- [x] **Hover / Pressed states for chip** (`ds:230:725`) — surface colours for Default / Hover / Pressed already match Figma (translucent → 0.95 → 1.0 white). The missing **drop-shadow elevation** landed 2026-05-25 as part of the Figma full sweep: new `AC-12c` *Suggestion chip elevation* (@evolving, anchored on `ds:230:725` parent set) covers the wider envelope on Default + Hover (`0 4 12 rgba(0,0,0,0.1)`) and the tighter envelope on Pressed (`0 4 6 rgba(0,0,0,0.1)`). Implementation in [`src/styles/suggestionChip.module.css`](src/styles/suggestionChip.module.css) — `box-shadow` transitions alongside `background` and respects `prefers-reduced-motion: reduce`.
- [x] **Hover / Pressed states for close button** (`ds:223:739`) — landed earlier. `CloseButton`'s Default has no fill, Hover uses `--gray-300`, Pressed uses `--gray-500`. Matches `ds:196:853` / `ds:223:740` / `ds:224:820`. (Was previously listed as a gap; corrected during 2026-05-25 Figma sweep.) The sidebar-item state cluster (`ds:230:452`) landed 2026-05-06 — see the next entry.
- [x] **Hover / Pressed states for `PreviousDiscussionItem` (`ds:230:452`)** — landed 2026-05-06; **active-row colour reconciled to Figma 2026-05-25** (Figma sweep). Row idle / hovered surfaces step `--gray-400` → `--gray-500` per the parent set's Default `ds:191:268` / Hover `ds:230:453` variants. The active conversation row carries the **Hover** variant's surface (`--gray-500`) as its idle treatment per `site:434:2424` (`property1="Hover"` for the active row in Figma's composition) — one step darker than an inactive Default row, matching Figma exactly. Label typography stays Everett Regular on every variant (the surface step is the cue; the earlier bold-label cue has been dropped). The Pressed variant (`--gray-600`, `ds:230:459`) remains documented in §2.5 row AC-33a but is not rendered as a static row state by the widget — reserved for a future transient "while pressing" surface if one is ever needed. Hovering an inactive row briefly produces the same surface as the active row, which is intentional: Figma's Hover variant doubles as the canonical "active-looking" treatment. AC-33a body amended (active row → Hover surface, bold-label cue dropped); §2.5 row AC-33a updated to add `site:434:2424` as a secondary anchor and bump `Last checked` to 2026-05-25.
